    ...
    for(let c=0; c < brickColumnCount; c++){
      ...
      for (let c = 0; c < brickRow;c++)
      {
        for (let r = 0; r < brickCol;r++)
        {
          ...
          {
            let brickX =  (c* (brickWidth + brickPadding)) +2.13*brickOffsetLeft;
            let brickY =  (r* (brickHeight+brickPadding)) + 8*brickOffsetTop;
      ...
      for(let c=0; c < brickColumnCount; c++){
            for(let r=0; r < brickRowCount; r++){
                if(lowBricks2[c][r].status === 1){
                    let brickX = ( c* (brickWidth + brickPadding)) +brickOffsetLeft;
                    let brickY = ( r* (brickHeight + brickPadding)) +12.2* brickOffsetTop;
...
function collisionDetection(){
  if (score === (3*(brickRowCount*brickColumnCount))+2*(brickRow*brickCol)){
